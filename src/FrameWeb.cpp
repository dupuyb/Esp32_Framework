...
#include "FrameWeb.h"
#ifdef PIO_SEEED_XIAO_ESP32C6_BOARD
#include <miniz.h>  
#include "lwip/pbuf.h"
#include "lwip/netif.h"
// Correctif temporaire pour le bug lwIP d'Espressif sur ESP32-C6
extern "C" int lwip_hook_ip6_input(struct pbuf *p, struct netif *inp) {
    return 0; // 0 indique à lwIP de continuer le traitement normal
}
#else
#include <rom/miniz.h>
#endif
...
//---- Start Generated from src/FrameWeb.html file --- 2026-08-19 15:25:10 | sha256=1265f822563af038a373fbf32e21c04ae22199d5df85c1d93716d3e2ccb8e671 | zip_mode=zip_if_best
...
void FrameWeb::loadConfiguration(const char *filename, Config &config, const char* hname) {
  ...
  if (!file)
    FDBXERR("File(%s) is absent.", filename);
  ...
  if ( error ) {
    FDBXLN(F("Force default configuration."));
...
void FrameWeb::showAH() {
	...
	m += "Nbr of headers:";	m+=server.headers();	m+="\n\r";
	for(int i = 0; i < server.headers(); i++) {
		m+="Header["+(String)i+"]="; m+=server.headerName(i)+":";	m+=server.header(i)+"\n\r";
	}
...
String FrameWeb::zipString(const String& input) {
  ...
  const int compressFlags = static_cast<int>(TDEFL_WRITE_ZLIB_HEADER) |
